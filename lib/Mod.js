...
class Mod {
  ...
  handleSubdir(dirname) {
    ...
    subDir.forEach(filename => {
      ...
      let pubFilepath = filepath.replace(this.group.webDirectory, "").replace(/\\/g, "/");
      //console.log(this.group.pubDirectory)
      debug("transpiling file:", pubFilepath);
      let contents = fs.readFileSync(filepath, 'utf8');

      this.ext.some(e => {
        if (e.check(filepath)) {
          let result = e.exec(pubFilepath, dirname, contents);
